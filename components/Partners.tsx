...
const Partners = ({ isStandalone = false }: { isStandalone?: boolean }) => {
  const partners = [
    { name: "Loxone", logo: "loxone.com" },
    { name: "Schneider", logo: "se.com" },
    { name: "ABB", logo: "abb.com" },
    { name: "Siemens", logo: "siemens.com" },
    { name: "Hager", logo: "hager.com" },
    { name: "Jablotron", logo: "jablotron.com" },
    { name: "Somfy", logo: "somfy.com" },
    { name: "Gira", logo: "gira.com" },
  ];

  return (
    <section className={`pt-12 md:pt-20 pb-16 md:pb-24 bg-transparent transition-colors duration-500 border-t border-black/5 dark:border-white/5 overflow-hidden ${isStandalone ? 'pt-32 md:pt-40' : ''}`}>
            ...
            >
              ...
              <div className="relative h-32 md:h-40 rounded-[2rem] bg-white dark:bg-white/5 border border-black/5 dark:border-white/10 flex flex-col items-center justify-center p-8 shadow-sm transition-all duration-500 group-hover:scale-[1.02] group-hover:shadow-xl group-hover:border-[#69C350]/30 group-hover:bg-white/80 dark:group-hover:bg-white/[0.08] backdrop-blur-sm">
                <div className="w-full h-12 md:h-16 flex items-center justify-center">
                  <img
                    src={`https://logo.clearbit.com/${p.logo}?size=200`}
                    alt={p.name}
                    className="max-w-full max-h-full object-contain filter brightness-100 dark:brightness-0 dark:invert transition-all duration-500 group-hover:scale-110"
                    onError={(e) => {
                      const target = e.target as HTMLImageElement;
                      target.src = `https://ui-avatars.com/api/?name=${p.name}&background=69C350&color=fff&bold=true`;
                    }}
                  />
                </div>
